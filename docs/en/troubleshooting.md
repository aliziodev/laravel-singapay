...
| **Only** money-out deliveries 401 while money-in is fine | Money-out notifications are signed by the **Default** credential while the app holds a Specific credential's secret | Declare the Default credential as a connection — every connection's secret is accepted during verification |
